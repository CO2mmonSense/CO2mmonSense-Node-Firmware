 ...
 */
    static void drawIconScreen(const char *upperMsg, OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x, int16_t y)
    {
        ...
        const char *title = USERPREFS_OEM_TEXT; // this is the OEM text from the user prefs instead of meshtastic.org
    ...
    static void drawOEMIconScreen(const char *upperMsg, OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x, int16_t y)
    {
        display->drawXbm(x + (SCREEN_WIDTH - icon_width) / 2,
                         y + (SCREEN_HEIGHT - FONT_HEIGHT_MEDIUM - icon_height) / 2 + 2, icon_width,
        ...
        display->setTextAlignment(TEXT_ALIGN_LEFT);
        // Replace title with USERPREFS_OEM_TEXT
        const char *title = "a Meshtastic project";
        display->drawString(x + getStringCenteredX(title), y + SCREEN_HEIGHT - FONT_HEIGHT_MEDIUM, title);
        display->setFont(FONT_SMALL);
        // Draw region in upper left
        if (upperMsg)
